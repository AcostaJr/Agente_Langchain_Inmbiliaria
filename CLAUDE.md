...
- **Pagos mensuales** de inquilinos en Google Sheets — cuota de gastos comunes/mantenimiento,
  con autenticación de dos factores (departamento + nombre del titular).
...
- **RF-13 (Pago mensual del inquilino con autenticación):** preguntas de un inquilino sobre
  **su propio** pago/cuota mensual (gastos comunes/mantenimiento) → tool
  `consultar_pago_inquilino`, que lee un Google Sheet de pagos (solo lectura, mismo service
  account). Es **información privada**: exige **dos factores** que el usuario debe aportar —
  número de departamento (`Bloque inmobiliario`) y nombre del titular — y solo revela el
  pago si **ambos coinciden con la misma fila**. Si no coinciden, **niega el acceso** con un
  mensaje genérico (no revela qué factor falló ni si el departamento existe → anti-enumeración)
  y **nunca** muestra datos de otros inquilinos. El desglose de la hoja cambia por mes.
...
│   └── google_sheets_pagos_inquilinos.py        # @tool consultar_pago_inquilino (Google Sheets + auth 2 factores)
├── rag/
...
- `consultar_pago_inquilino(departamento: str = "", nombre_inquilino: str = "") -> str` —
  Google Sheets de pagos vía `gspread` (mismo service account, scope readonly). **Auth de dos
  factores:** si falta `departamento` o `nombre_inquilino`, pide ambos; localiza la fila por
  número de departamento (columna A `Bloque inmobiliario`, único; ignora filas no numéricas
  como el `Total` final) y verifica el nombre del titular (columna B) en **esa misma fila**
  (comparación tolerante: sin acentos, minúsculas, admite nombre parcial). Solo con ambos
  correctos devuelve el desglose (columnas `Total …`, `SubTotal`, `Descuentos`) y el
  `Total` a pagar, con el período tomado del título del sheet. Denegación genérica en
  cualquier otro caso (anti-enumeración). Cabecera efectiva = fila 2 con fallback a fila 1
  (la hoja usa celdas combinadas). Cliente perezoso como las demás tools de Sheets.
...
| `GOOGLE_SHEETS_PAGOS_SPREADSHEET_ID` | ✅ (para `consultar_pago_inquilino`) | ID del Google Sheet de pagos mensuales |
| `GOOGLE_SHEETS_PAGOS_WORKSHEET` | ➖ (default: primera hoja) | Pestaña de la hoja de pagos (ej. `Rio Sul`) |
...
- **CA-12:** "¿Cuánto pago este mes?" → el agente pide departamento y nombre; con ambos correctos
  (ej. `101` + `Vinicius Torrealva`) `consultar_pago_inquilino` devuelve el desglose y el total.
  Con datos que no coinciden (nombre errado, o departamento de otro), niega el acceso y **no**
  revela el pago de otro inquilino (RF-13).
